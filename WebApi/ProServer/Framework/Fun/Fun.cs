...
namespace ProServer
{
    public class Fun
    {
        public static int GetSeqID<T>() where T : new()
        {
            ...
            {
                T tmp = new T();
                string sql = "SELECT nextval('" + tmp.GetType().Name + "_SEQ')";
                ...
                switch (dbType)
                {
                    ...
                    case "Oracle":
                        ...
                        break;
                    case "MySql":
                        sql = "SELECT nextval('" + tmp.GetType().Name + "_SEQ')";
                        break;
                    case "Sql":
                        return 0;
                }
        ...
        public static int GetCurrvalSeqID<T>() where T : new()
        {
            ...
            {
                T tmp = new T();
                string sql = "SELECT   " + tmp.GetType().Name + "_SEQ.CURRVAL   FROM   DUAL";
                string dbType = GetDataBaseType();
                switch (dbType)
                {
                    case "DB2":
                        sql = "SELECT   " + tmp.GetType().Name + "_SEQ.CURRVAL   FROM   SYSIBM.DUAL";
                        break;
                    case "Oracle":
                        sql = "SELECT   " + tmp.GetType().Name + "_SEQ.CURRVAL   FROM   DUAL";
                        break;
                    case "MySql":
                        sql = "SELECT CURRVAL('" + tmp.GetType().Name + "_SEQ')";
                        break;
                    case "Sql":
                        return 0;
                }
        ...
        public static string UrlToAllPath(string url)
        {
            ...
            url = url.Replace("\"", "");
            string urlPath =  ProInterface.AppSet.ConfigWebUrl;
        ...
        {
            string WebUrl = ProInterface.AppSet.ConfigWebUrl;
            string WebRootPath = System.AppDomain.CurrentDomain.BaseDirectory;
        ...
        public static string AbsoluteUrl(string url)
        {
            if (url.IndexOf("http") == -1)
            {
                string urlPath =  ProInterface.AppSet.ConfigAbsolute;
                url = urlPath + url.Replace("../", "").Replace("~/", "");
            }
            return url;
        }
